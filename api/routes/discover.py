...
def search_semantic_scholar(body: SearchRequest):
    ...
    """
    from lib.semantic_scholar import search_papers, format_paper_for_display, check_papers_in_library
...
def add_from_discover(body: AddPaperRequest):
    ...
    try:
        ...
        upsert_paper(filename, {
            ...
            'authors': crossref.get('authors', [authors_str] if authors_str else []),
        ...
        return {
            'success': True,
            'message': f"Added: {title[:50]}..." if len(title) > 50 else f"Added: {title}",
            'pdf_found': pdf_found,
        }
...
def _analyze_reference_gaps_pg(limit: int = 50) -> tuple[list[dict], dict]:
    ...
    with get_session() as session:
        ...
        rows = session.query(Paper.doi, Paper.title).filter(
        ...
        refs = session.query(
            ...
            Paper.title,
        ).join(Paper, Paper.filename == PaperReference.paper_filename).filter(
            Paper.deleted_at.is_(None)
        ).all()
