...
def get_api_key():
    ...
    if not api_key:
        api_key = session_state.get("anthropic_api_key")
    if not api_key:
        if True:  # Was st.sidebar
            logger.warning("⚠️ Anthropic API key required for queries")
            api_key = input(
            ...
            if api_key:
                session_state.anthropic_api_key = api_key
                pass  # Was st.rerun()
    return api_key
...
def query_crossref_for_metadata(doi: str) -> dict:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Abstract
            from lib.jats import strip_jats
            metadata['abstract'] = strip_jats(message.get('abstract', ''))
...
def import_csv_papers(csv_papers: list, batch_size: int, skip_existing: bool, existing_papers: list):
    ...
    if csv_papers:
        ...
        logger.info(f"📋 **Import Source Detected:** {source_type.replace('_', ' ').title()}")
    ...
    for batch_num in range(num_batches):
        ...
        for idx_in_batch, csv_paper in enumerate(batch_papers):
            ...
            try:
                ...
                logger.info(f"Importing: {global_idx + 1}/{total_papers} ({imported} imported, {skipped} skipped, {failed} failed)")
    ...
    if imported > 0:
        ...
        logger.warning("No papers to import")
    ...
    if imported > 0:
        ...
        sys.stderr.flush()
        logger.info("✅ Import complete! Refreshing app to show new papers...")
        time.sleep(2)
        sys.stderr.write("[BATCH LOOP] Calling rerun now...\n")
        sys.stderr.flush()
        pass  # Was st.rerun()
    else:
...
def main():
    ...
    if "selected_paper" not in session_state:
        session_state.selected_paper = None
    if "query_result" not in session_state:
        session_state.query_result = None
    if "active_tab" not in session_state:
        session_state.active_tab = "Library"

    # Initialize theme
    if "theme" not in session_state:
        session_state.theme = load_theme_preference()
    ...
    current_theme = session_state.theme
    logger.debug(styles.get_professional_css(current_theme), unsafe_allow_html=True)
    ...
    if 'cached_papers' not in session_state or session_state.get('reload_papers', False):
        try:
            session_state.cached_papers = rag.get_paper_library()
            session_state.cached_filter_options = rag.get_filter_options()
            session_state.cached_total_chunks = rag.get_collection_count()
            session_state.reload_papers = False
        except (FileNotFoundError, RuntimeError) as e:
            logger.error(str(e))
            logger.info("Please run `python scripts/ingest.py` first to create the database")
            pass  # Was st.stop()

    papers = session_state.cached_papers
    filter_options = session_state.cached_filter_options
    total_chunks = session_state.cached_total_chunks
    ...
    if True:  # Was st.sidebar
        ...
        logger.info("Total Papers", total_papers)
        ...
        logger.debug(complete_pct / 100, text=f"✅ Complete: {complete_papers}")
        logger.debug(metadata_pct / 100, text=f"📋 Metadata Only: {metadata_only_papers}")
        logger.debug(incomplete_pct / 100, text=f"⚠️ Incomplete: {incomplete_papers}")
        logger.info("Chunks", total_chunks)

    # Main content - Tabs
    pass  # Was st.tabs(["📥 Import", "Library", "🔍 Discover", "Research", "History", "Settings", "🗑️ Trash"])
