...
class DatabaseClient:
    ...
    def get_collection(cls):
        """Load and cache the ChromaDB collection (thread-safe)."""
...
def _ensure_list(val):
    ...
    Handles the case where metadata stores a string like 'NMC'
    instead of a list ['NMC'] — prevents character-by-character iteration."""
    if val is None:
        return []
    if isinstance(val, list):
        return val
    if isinstance(val, str):
        # Split on comma/semicolon to handle 'NMC, LFP' -> ['NMC', 'LFP']
...
def get_paper_library() -> list[dict]:
    ...
    if metadata_file.exists():
        with open(metadata_file, 'r', encoding='utf-8') as f:
            ...
            # Update existing papers with date_added, pdf_status, feed_blurb, and paper_type
            for paper in papers.values():
                ...
                if filename in full_metadata:
                    ...
                    # paper_type from metadata.json is authoritative
                    pt = full_metadata[filename].get('paper_type', '')
                    if pt:
                        paper['paper_type'] = pt
                else:
                    paper['date_added'] = ''
            ...
            for filename, meta in full_metadata.items():
                if filename not in papers:
                    ...
                    papers[filename] = {
                        ...
                        'paper_type': meta.get('paper_type', 'Review'),
            ...
            # Filter out deleted papers and ghost ChromaDB entries not in metadata.json
            papers = {k: v for k, v in papers.items()
                      if k in full_metadata and not full_metadata[k].get('deleted_at')}
...
def get_filter_options() -> dict:
    ...
    for metadata in all_results['metadatas']:
        if metadata.get('chemistries'):
            chemistries.update(c.strip() for c in metadata['chemistries'].split(','))
        if metadata.get('topics'):
            topics.update(t.strip() for t in metadata['topics'].split(','))

    # Get paper_types from metadata.json (authoritative source)
    paper_types = set()
    metadata_file = Path("data/metadata.json")
    if metadata_file.exists():
        with open(metadata_file, 'r', encoding='utf-8') as f:
            full_metadata = json.load(f)
        for paper in full_metadata.values():
            pt = paper.get('paper_type', '').strip()
            if pt:
                paper_types.add(pt)

    return {
        'chemistries': sorted([c for c in chemistries if c and len(c) >= 2]),
        'topics': sorted([t for t in topics if t and len(t) >= 2]),
        'paper_types': sorted([p for p in paper_types if p])
    }
    ...
    if results['documents']:
        ...
        with open(metadata_file, 'r', encoding='utf-8') as f:
            ...
            details = {
                ...
                'paper_type': paper_meta.get('paper_type', 'Review'),
